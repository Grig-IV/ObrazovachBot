...
class DatabaseManager:
    ...
    def create_db_message(self, user):
        ...
        with open('PlaceholderFile', 'rb') as db_file:
            ...
            user.set_data('db_message_id', db_message_id)

    def load_db(chat_id, db_message_id):
        ...
        db_str = requests.get(FILE_URL.format(tb.token, file_path)).text
    ...
    def save_db(self, users, db_dict):
        ...
        with open('bot_database.json', 'rb') as db_file:
            for user in users:
                db_message_id = user.data.get('db_message_id')
